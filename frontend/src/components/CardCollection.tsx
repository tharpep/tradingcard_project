...
export function CardCollection({ onLogout }: CardCollectionProps) {
  ...
  const handleAddCard = async (e: React.FormEvent) => {
    ...
    try {
      setIsAdding(true)
      setError(null)
      await api.addCard({ name: newCardName.trim() })
      setNewCardName('')
      await loadCards()
    } catch (err) {
  ...
  const handleToggleFavorite = async (card: CardType) => {
    try {
      setError(null)
      await api.updateCard(card.id, { is_favorite: !card.is_favorite })
      await loadCards()
    } catch (err) {
